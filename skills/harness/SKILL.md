...
| **멀티런타임** (선택) | 모델 다양성이 필요한 교차 검증·2nd opinion·독립 재현. **사용자가 외부 런타임을 명시적으로 요청할 때만** | 네이티브 팀 + 어댑터 에이전트가 `codex`/`agy`에 읽기 전용 위임 |
...
4. **멀티런타임은 사용자가 외부 런타임을 언급했을 때만 검토한다.** 언급이 없으면 기본값은 Claude 전용 팀이다 — 멀티런타임을 기본으로 삼으면 모든 하네스가 외부 CLI 설치·인증에 의존하고 비용·지연이 예측 불가능해진다. 외부 CLI는 팀원이 될 수 없으므로(`SendMessage`/`TaskCreate` 참여 불가) 반드시 어댑터 에이전트를 경유한다

> 멀티런타임 상세(런타임 레지스트리, 어댑터 패턴, 검증 규약, 읽기 전용 원칙)는 `references/multi-runtime-guide.md` 참조.
...
**어댑터 에이전트 포함 시 필수 사항 (멀티런타임 모드):**
- 어댑터는 `general-purpose` 타입을 쓴다 (`delegate.py` 실행에 Bash가 필요하므로 읽기 전용인 `Explore`는 불가)
- 에이전트 정의에 `## 위임 프로토콜` 섹션을 추가하고 런타임·모델·권한·종료 코드 분기를 명시한다
- 어댑터는 외부 결과를 요약·윤색하지 않고 **출처를 병기해 원문을 전달**한다. 다른 팀원 결론에 맞춰 조정하면 교차 검증의 의미가 사라진다
- 상세 가이드와 정의 템플릿: `references/multi-runtime-guide.md` 참조
...
| **외부 런타임** | 어댑터가 `delegate.py`로 위임하고 파일로 회수 | 멀티런타임 | **파일 기반 전용** — 외부 CLI는 메시지·태스크·반환값 전략을 쓸 수 없다 |
...
- **멀티런타임**: 대상 CLI가 PATH에 있고 인증되어 있는지 사전 점검한다 (`delegate.py --dry-run`으로 조립된 명령줄 확인). 읽기 전용 플래그(`codex --sandbox read-only`, `agy --sandbox`)가 실제로 걸리는지, 외부 CLI가 팀원으로 잘못 배치되지 않았는지 확인
...
- [ ] 실행 모드 명시 (에이전트 팀 / 서브 에이전트 / 하이브리드 / 멀티런타임 중 선택, 하이브리드면 Phase별 모드 기재)
- [ ] 멀티런타임 사용 시 — 어댑터 에이전트 정의 생성, 읽기 전용 플래그 확인, 외부 CLI를 팀원으로 배치하지 않았는지 확인
...
- **멀티런타임 가이드**: `references/multi-runtime-guide.md` — 외부 CLI 에이전트(`codex`, `agy` 등) 통합. 런타임 레지스트리, 어댑터 에이전트 패턴, 결과 검증 규약, 읽기 전용 원칙, 비용·지연. 번들 스크립트 `scripts/delegate.py`와 함께 사용
