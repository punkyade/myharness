...
1. 실행 모드: 에이전트 팀 / 서브 에이전트 / 멀티런타임
2. 에이전트 팀 아키텍처 유형 (6패턴)
3. 복합 패턴
4. 에이전트 타입 선택
5. 에이전트 정의 구조
6. 에이전트 분리 기준
...
### 멀티런타임 (Multi-runtime) — 선택 모드

네이티브 팀에 **어댑터 에이전트**를 넣어, 외부 CLI 에이전트(`codex`, `agy` 등)에 작업을 위임하고 결과를 회수한다. 목적은 단 하나 — **모델 다양성**이다. 팀원이 전부 Claude Code 인스턴스면 아무리 교차 검증을 해도 같은 모델의 같은 맹점을 공유한다.

```
[리더] ←→ [팀원A] ←→ [codex-어댑터] ←→ [agy-어댑터]
                          ↓                 ↓
                    delegate.py (읽기 전용)
                          ↓                 ↓
                    [codex exec]      [agy -p]
...
**결정적 제약:** 외부 CLI는 **팀원이 될 수 없다.** `TeamCreate`는 Claude Code 인스턴스를 스폰하므로, `codex`/`agy`는 `SendMessage`를 받지 못하고 `TaskCreate` 공유 목록에도 참여하지 못한다. `TeamCreate`의 members에 외부 CLI를 넣으면 생성된 하네스가 실행 시점에 조용히 깨진다. 반드시 네이티브 어댑터를 경유한다.

**제약:**
- 외부 호출 1건 = 완전한 에이전트 실행 (비용·지연 큼)
- 파일 기반 전달 전용 — 메시지·태스크·반환값 전략 사용 불가
- 외부 런타임은 읽기 전용. 파일 수정은 네이티브 에이전트가 단일 진입점으로 수행
- 대상 CLI의 설치·인증에 의존 — 사전 점검 필수

> 상세: `multi-runtime-guide.md` (런타임 레지스트리, 어댑터 템플릿, 검증 규약)

### 모드 선택 의사결정 트리

```
사용자가 외부 런타임(codex/agy 등)을 명시적으로 요청했는가?
├── Yes → 모델 다양성이 실제로 이득인가?
│         (교차 검증·2nd opinion·독립 재현 → Yes / 단순 생성·편집 → No)
│         ├── Yes → 멀티런타임 (네이티브 팀 + 어댑터 에이전트)
│         └── No  → 아래 기본 분기로. 사용자에게 이유를 설명한다.
│
└── No → 에이전트가 2개 이상인가?
         ├── Yes → 에이전트 간 통신이 필요한가?
         │         ├── Yes → 에이전트 팀 (기본값)
...
> **핵심 원칙:** 에이전트 팀이 기본이다. 서브 에이전트를 선택할 때는 "팀원 간 통신이 정말 불필요한가?"를 자문한다. 멀티런타임은 요청이 있을 때만 검토한다 — 기본값으로 삼으면 모든 하네스가 외부 CLI에 의존하게 된다.
