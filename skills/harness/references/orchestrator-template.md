...
- **템플릿 D: 멀티런타임 모드 (선택)** — 외부 CLI로 모델 다양성 확보, 명시적 요청 시에만

---

## 목차

1. 템플릿 A: 에이전트 팀 모드 (기본)
2. 템플릿 B: 서브 에이전트 모드 (대안)
3. 템플릿 C: 하이브리드 모드
4. 템플릿 D: 멀티런타임 모드 (선택)
5. 작성 원칙
6. description 작성 시 후속 작업 키워드
7. 실제 오케스트레이터 참고
...
**사용자가 외부 런타임을 명시적으로 요청했을 때만 쓴다.** 모델 다양성이 필요한 교차 검증·2nd opinion 용도다.

핵심 제약: **외부 CLI는 팀원이 될 수 없다.** `TeamCreate`는 Claude Code 인스턴스를 스폰하므로 `codex`/`agy`는 `SendMessage`·`TaskCreate`에 참여하지 못한다. 반드시 네이티브 어댑터 에이전트를 경유한다. 상세는 `multi-runtime-guide.md` 참조.

```markdown
---
name: {domain}-orchestrator
description: "{도메인} 멀티런타임 에이전트 팀을 조율하는 오케스트레이터. {초기 실행 키워드}. 교차 검증, 2nd opinion, codex/agy 활용 요청 시. 후속 작업: 결과 수정, 부분 재실행, 업데이트, 보완, 다시 실행 요청 시에도 반드시 이 스킬을 사용."
---

## Phase 0: 컨텍스트 확인 + 런타임 사전 점검

1. `_workspace/` 존재 여부로 초기/후속/부분 재실행 판별
2. **사전 점검** — 사용할 외부 CLI가 실제로 쓸 수 있는지 확인한다:
   ```bash
   python ${CLAUDE_PLUGIN_ROOT}/skills/harness/scripts/delegate.py \
     --runtime {runtime} --prompt-file /dev/null --out /dev/null --dry-run
   ```
   CLI가 없으면(종료 코드 3) 사용자에게 알리고 **해당 런타임 없이 진행할지 확인받는다.**
   조용히 건너뛰면 사용자는 교차 검증이 수행된 것으로 착각한다.
3. **예상 외부 호출 횟수를 사용자에게 미리 알린다** (대상 건수 × 런타임 수)

## Phase 1: 팀 구성

TeamCreate(members):
  - {분석가}       : 네이티브, 1차 산출물 생성
  - {runtime}-adapter : 네이티브 어댑터, {runtime}에 독립 검토 위임
  - {synthesizer}  : 네이티브, 결과 통합 및 불일치 정리

## Phase 2: 병렬 실행

- 네이티브 팀원: 평소대로 작업
- 어댑터: 자기완결적 프롬프트 작성 → delegate.py 위임 → 회수·검증
  → 출처를 병기해 팀에 반환

산출물 경로 규약 (출처 보존 필수):
  _workspace/{phase}_{agent}@{runtime}_{artifact}.{ext}

## Phase 3: 통합

- 런타임 간 **합의 항목**과 **불일치 항목**을 분리해 정리한다
- 불일치는 삭제하지 않고 양쪽을 출처와 함께 병기한다 —
  불일치 자체가 교차 검증의 핵심 산출물이다
- 외부 런타임이 실패해 누락된 항목이 있으면 보고서에 명시한다

## 에러 핸들링

| 상황 | 조치 |
|------|------|
| 종료 코드 2 (타임아웃) | 1회 재시도. 재실패 시 부분 결과를 "부분"으로 명시해 사용 |
| 종료 코드 3 (CLI 부재·인증 실패) | 재시도하지 않음. 누락으로 기록하고 나머지 런타임으로 진행 |
| 전체 외부 런타임 실패 | 네이티브 결과만으로 진행하되, **교차 검증이 수행되지 않았음을 보고서 상단에 명시** |
| 런타임 간 결론 상충 | 정상 상황이다. 양쪽 병기 + 판단 근거 비교를 산출물에 포함 |
```

**멀티런타임 설계 시 확인:**
- 외부 CLI가 `TeamCreate` members 목록에 들어가 있지 않은가 (들어가면 실행 시 조용히 깨진다)
- 어댑터가 `general-purpose` 타입인가 (`Explore`는 Bash 불가)
- 읽기 전용인가 — 파일 수정은 네이티브 에이전트가 단일 진입점으로 수행
- 모델 다양성이 실제로 확보되는가 — `agy --model claude-*`를 쓰면서 다양성을 주장하면 자기기만이다

---

## 작성 원칙

1. **실행 모드를 먼저 명시** — 오케스트레이터 상단에 "에이전트 팀" / "서브 에이전트" / "하이브리드" / "멀티런타임" 중 하나 명시. 하이브리드면 Phase별 모드 표 필수, 멀티런타임이면 사용 런타임과 사전 점검 단계 필수
